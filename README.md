...
将模型升级为策略，遍历沪深300和中证500的标的，先做一遍标的筛选，除去一些模型准确度不合格的标的，因此先运行filter_stock.py，里面训练集选取的日期可以根据实际修改。filter_stock.py没有多线程跑，运行时间相对较长，大概是6个小时上下，尝试用了线程池去优化，但是python一个进程最多只有一个cpu做处理，所以对于计算密集型的部分优化不明显，之后会尝试用多进程优化一下。对于下载数据方面尝试用了多线程下载，可是本身baostock分配给一个ip的服务是单线的，所以用多线程下载数据会报错。
