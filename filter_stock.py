...
def getLatestCode():
    ...
    bs.login()
    # 获取沪深300和中证500成分股
    rs_hs300 = bs.query_hs300_stocks(boundary_date)
    rs_zz500 = bs.query_zz500_stocks(boundary_date)
    # 打印结果集
    all_stocks = []
    ...
    filter_df = pd.DataFrame(all_stocks, columns=rs_hs300.fields)

    with ThreadPoolExecutor(max_workers=1) as t:  # 线程池下载数据 目前是单线程
        for i in range(len(filter_df["code"])):
            stockname = filter_df["code"][i]
            t.submit(downloadStock, stockname)
        t.shutdown()
    bs.logout()  # 登出系统

    for i in range(len(filter_df["code"])):
        targetCode = filter_df["code"][i]
        targetCode_df = pd.read_csv(output_Folder + targetCode + ".csv", encoding="gbk")
        if len(targetCode_df) == 0:
            P_ud = 0
        else:
            # 删除掉近期三年内上市的股票
            if datetime.datetime.strptime(targetCode_df['date'][0], '%Y-%m-%d').date() > (
                    datetime.datetime.strptime(boundary_date, '%Y-%m-%d').date() + relativedelta(years=-5)):
                P_ud = 0
            else:
                P_ud = verify_output(targetCode_df)
        filter_df.loc[i, 'P_ud'] = P_ud
    filter_df = filter_df.drop(index=(filter_df.loc[(filter_df['P_ud'] <= P_line)].index))
    # 结果集输出到csv文件
    filter_df.to_csv(output_Folder + "filter.csv", encoding="gbk", index=False)


def downloadStock(stockname):
    filePath = output_Folder + stockname + ".csv"
    bear_date = datetime.date.today() + datetime.timedelta(days=-250)  # 设定一年刷新
    # 一次数据
    if os.path.exists(filePath) and bool(bear_date.strftime('%Y-%m-%d') < time.strftime('%Y-%m-%d',
                                                                                        time.localtime(
                                                                                            os.stat(
                                                                                                filePath).st_mtime))):
        print(stockname, '数据已下载')

    else:
        rs = bs.query_history_k_data_plus(stockname, "date,code,open,high,low,close,preclose,volume,amount,pctChg",
                                          start_date=start_date, end_date=boundary_date, frequency="d")
        # 打印结果集
        data_list = []
        ...
        # 结果集输出到csv文件
        result.to_csv(output_Folder + stockname + ".csv", encoding='gbk', index=False)
        print(stockname, '数据已重新下载/更新')
...
def verify_output(df):
    VOend_date = datetime.datetime.strptime(boundary_date, '%Y-%m-%d').date()
    VOstart_date = VOend_date + relativedelta(years=-2) + datetime.timedelta(
        days=1)  # 选择回测的范围 由于调至两年或者三年运行时间过长，因此偷工减料到1年
